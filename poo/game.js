...
class Game {
  ...
  onMouseMove (e) {
    ...
    const x = (e.offsetX / this.RESOLUTION) | 0
    const y = (e.offsetY / this.RESOLUTION) | 0
    const cell = this.getEntityAt(x, y)
    if (cell && cell.isDead) {
      cell.procreate()
    }
  }
  ...
  setup (level) {
    ...
    for (let x = 0; x < level.length; x++) {
      for (let y = 0; y < level[x].length; y++) {
        const cell = new Cell('Cell' + (x + y), y, x)
  ...
  getEntityAt (x, y) {
    if (this.levelEntitiesPositions[x] && this.levelEntitiesPositions[x][y] && this.levelEntitiesPositions[x][y]) { return this.levelEntitiesPositions[x][y][0] } else { return undefined }
